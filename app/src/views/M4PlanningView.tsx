...
export function M4PlanningView({ data, initialKind = "all" }: { data: MoveMapData; initialKind?: ItemKind | "all" }) {
  ...
  const archivedIncome = useMemo(() => incomeRows.filter((row) => row.archived), [incomeRows]);
  const archivedHousing = useMemo(() => housingRows.filter((row) => row.archived), [housingRows]);
